...
DrinkOrder.find({status:"complete"}).sort('date').exec(function(err, orders) {
  if (err) {
    ...
    for(var i = 0;i<2;i++) {
      ...
      console.log("Barista %s completed %s orders, with an average of start-to-start time of %s minutes",i,baristaOrderCounts[i], ((baristaStartToStartTimes[i]/baristaOrderCounts[i])/1000/60).toFixed(2));
    }
    console.log("Average overall startToStart time is %s minutes", (overallStartToStart/overallStartToStartCount/1000/60).toFixed(2))
    ...
    console.log("Average time to make drinks:",(averageDuration/60).toFixed(2),"minutes");
    var averageOverallStartToStartSeconds = (overallStartToStart/overallStartToStartCount)/1000
    console.log("Average time between drinks: %s mins",((averageOverallStartToStartSeconds - averageDuration) / 60).toFixed(2))

    var averageWaitEstimateDelta = totalWaitTimeDeltas / totalWaitTimeOrders;
    console.log("***** ESTIMATE ACCURACY *****")
    console.log("On average, the estimates generated by the system were %s minutes %s.", Math.abs(averageWaitEstimateDelta).toFixed(2), averageWaitEstimateDelta < 0 ? "over" : "under")  
  }
  process.exit();
});
